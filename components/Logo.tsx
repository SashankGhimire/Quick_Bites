...
const Logo: React.FC<{ className?: string }> = ({ className = "" }) => {
  return (
    <div className={`flex items-center group ${className}`}>
      ...
      <span
        className={`text-2xl font-bold tracking-tight ${className.includes("text-white") ? "text-white" : "text-slate-800"}`}
      >
        Quick
        <span className={className.includes("text-white") ? "text-white" : "text-primary-600"}>
          Bite
        </span>
      </span>
    </div>
